...
function solution(a, b, c, d) {
    ...
    // 세 개가 같고, 하나가 다를 경우 ((10 * p + q)^2)
    if (freq.includes(3)) {
        const p = uniqueValues[freq.indexOf(3)];
        const q = uniqueValues[freq.indexOf(1)];
        return (10 * p + q) ** 2;
    }

    // 두 쌍의 숫자가 같은 경우 ((p + q) * |p - q|)
    if (freq.length === 2 && freq[0] === 2 && freq[1] === 2) {
        const [p, q] = uniqueValues;
        return (p + q) * Math.abs(p - q);
    }

    // 두 개가 같고 나머지 두 개가 다른 경우 (q * r)
    if (freq.includes(2) && freq.length === 3) {
        const q = uniqueValues[freq.indexOf(1)];
        const r = uniqueValues[freq.lastIndexOf(1)];
        return q * r;
    }

    // 모든 숫자가 다를 경우 (가장 작은 숫자)
